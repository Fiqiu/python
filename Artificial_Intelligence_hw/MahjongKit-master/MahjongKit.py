...
def is_yipai(tile_me):
    if tile_me>=31 and tile_me<=33:
        return True
    return False

def is_bianzhang(tile_me):
    if tile_me==0 or tile_me ==1 or tile_me ==7 or tile_me==8:
        return True
    if tile_me==9 or tile_me ==10 or tile_me ==16 or tile_me==17:
        return True
    if tile_me==18 or tile_me ==19 or tile_me ==25 or tile_me==26:
        return True
    return False    

"""def is_bianzhang(tile_me):
    if tile_me==0 or  tile_me==8:
        return True
    if tile_me==9 or  tile_me==17:
        return True
    if tile_me==18 or  tile_me==26:
        return True
    return False    """


def is_feiyifangpai(tile_me,tile_zifeng,tile_changfeng):
    if tile_me==tile_zifeng or tile_me==tile_changfeng:
        return True
    return False

def is_zhongzhang(tile_me):
    if tile_me >=2 and tile_me<=6:
        return True
    if tile_me >=11 and tile_me<=15:
        return True
    if tile_me >=20 and tile_me<=24:
        return True
    return False


def is_feiyifengpai(tile_me,tile_zifeng,tile_changfeng):
    if tile_me<=26 or tile_me>=31:
        return False
    if tile_me ==tile_zifeng or tile_me==tile_changfeng:
        return False
    return True
...
def bianzhang_number(tile_me_zong):
    bianzhang=0
    for i in tile_me_zong:
        if is_bianzhang(i)==True:
            bianzhang=bianzhang+1
    return bianzhang



def yipai_number(tile_me_zong,tile_zifeng,tile_changfeng):
    yipai=0
    for i in tile_me_zong:
        if is_yipai(i)==True:
            yipai=yipai+1
        if is_feiyifangpai(i,tile_zifeng,tile_changfeng)==True:
            yipai=yipai+1
    return yipai

def feiyifengpai_number(tile_me_zong,tile_zifeng,tile_changfeng):
    feiyipai=0
    for i in tile_me_zong:
        if is_feiyifengpai(i,tile_zifeng,tile_changfeng)==True:
            feiyipai=feiyipai+1
    return(feiyipai)


def zhongzhang_numbei(tile_me_zong):
    zhongzhang=0
    for i in tile_me_zong:
        if is_zhongzhang(i)==True:
            zhongzhang=zhongzhang+1
    return zhongzhang
...
def is_tingpai(tile_me_hand,tile_me_meld):
    jkj=Partition()
    if jkj.shantin_normal(tile_me_hand,tile_me_meld)==0:
        return 1
    if jkj.shantin_no_triplets(tile_me_hand,tile_me_meld,[30])==0:
        return 1
    if jkj.shantin_no_19(tile_me_hand,tile_me_meld)==0:
        return 1
    if jkj.shantin_no_sequences(tile_me_hand,tile_me_meld)==0:
        return 1
    if jkj.shantin_seven_pairs(tile_me_hand,tile_me_meld)==0:
        return 1
    if jkj.shantin_pure_color(tile_me_hand,tile_me_meld)==0:
        return 1
    return 0
...
def xiangtingshu(tile_me_hand,tile_me_meld):
    jkj=Partition()
    zhengchangx=jkj.shantin_normal(tile_me_hand,tile_me_meld)
    notrx=jkj.shantin_no_triplets(tile_me_hand,tile_me_meld,[30])
    no19x=jkj.shantin_no_19(tile_me_hand,tile_me_meld)
    nosex=jkj.shantin_no_sequences(tile_me_hand,tile_me_meld)
    sencx=jkj.shantin_seven_pairs(tile_me_hand,tile_me_meld)
    purx=jkj.shantin_pure_color(tile_me_hand,tile_me_meld)
    return min(zhengchangx,notrx,no19x,nosex,sencx,purx)
...
def main():
    glc = GameLogCrawler()
    """glc.batch_crawl_refids(17, ite=3)
    glc.batch_crawl_levels(ite=3)
    glc.batch_crawl_logs(17, ite=3)"""
    zongshuju=[]
    gene = glc.db_get_logs_where_players_lv_gr(17)
    #zongshuju = gene.__next__()
    #ooo=0
    for logg in gene:
        #if ooo>=1:
            #break
        ...
        log = gene.__next__()
        try:
            ress = PreProcessing.process_one_log(log)
        except:
            continue
        #ooo=ooo+1
        ...
        zongshuju=zongshuju+[ress]
    data_set_majiang=[]
    majdataset=[]
    xumupaduan=[]
    kjkj=0
    for res in zongshuju:
        for k, v in res.items():
            mingzi=[]
            fenshu=[]
            dangqianxiangtingshu=[7,7]
            #print("Round {}".format(k))
            for state in v:
                
                try:
                    binzhangshu=bianzhang_number(state.s_discard34)
                except:
                    continue
                if state.name not in mingzi:
                    mingzi=mingzi+[state.name]
                    fenshu=fenshu+[state.score]
                if state.name in mingzi:
                    for i in range(len(mingzi)):
                        if state.name==mingzi[i]:
                            fenshu[i]=state.score

                ZZZ=zip(fenshu,mingzi)
                ZZZ=sorted(ZZZ,reverse=True)
                #print(ZZZ)
                #print("sss")
                fenshu_new,mingzi_new=zip(*ZZZ)
                zongdeshuwei=[]
                for iii in range(len(fenshu_new)):
                    zongdeshuwei=zongdeshuwei+[[(iii+1),mingzi_new[iii],fenshu_new[iii]]]
                    if state.name==mingzi_new[iii]:
                        dangshunweiben=iii+1


                #print(zongdeshuwei)
                dangqianxiangtingshu[0]=dangqianxiangtingshu[1]
                dangqianxiangtingshu[1]=xiangtingshu(state.s_hand34,state.s_meld34)
                xunmushu=is_xunmu(len(state.s_discard34))
                if xunmushu>=1:
                    if dangqianxiangtingshu[1]>dangqianxiangtingshu[0]:
                        if dangshunweiben!=4:
                            xumupaduan=xumupaduan+[[(zongdeshuwei[dangshunweiben-1][2]-zongdeshuwei[dangshunweiben][2]),dangshunweiben,1]]
                if xunmushu>=1:
                    if dangqianxiangtingshu[1]<=dangqianxiangtingshu[0]:
                        if dangshunweiben!=4:
                            xumupaduan=xumupaduan+[[(zongdeshuwei[dangshunweiben-1][2]-zongdeshuwei[dangshunweiben][2]),dangshunweiben,0]]


                """print(dangqianxiangtingshu)
                print(mingzi_new)
                print(fenshu_new)
                print(fenshu)
                ...
                print(state.dan)
                print(state.score)
                print(xiangtingshu(state.s_hand34,state.s_meld34))"""

                yipaishu=yipai_number(state.s_discard34,state.s_player_wind,state.s_round_wind)
                feiyipaishu=feiyifengpai_number(state.s_discard34,state.s_player_wind,state.s_round_wind)
                zhongzhangshu=zhongzhang_numbei(state.s_discard34)
                xunmushu=len(state.s_discard34)
                fulushu=len(state.s_meld34)
                shifouting=is_tingpai(state.s_hand34,state.s_meld34)
                majdataset=majdataset+[[binzhangshu,yipaishu,feiyipaishu,zhongzhangshu,xunmushu,fulushu,shifouting]]
    ...
    # prepare model
    iiio=0
    """while iiio < len(trainingSet):
        if trainingSet[iiio][4]==0:
            trainingSet.pop(iiio)
            iiio=iiio-1
        iiio=iiio+1"""
    summaries = summarizeByClass(trainingSet)
    #summaries={0: [(1.5259224107248783, 1.4649632321766475), (1.4329101910072368, 1.3872284861081143), (0.7948748368727014, 0.9032601057728333), (1.085211768893107, 1.604403943857613), (0.6458654644679084, 0.7794565625298573), (0.20118044845177363, 0.4806316293242457)], 1: [(2.5357981220657275, 1.5730332610079065), (2.272300469483568, 1.450711886807226), (1.099618544600939, 0.987624004916177), (3.1993838028169015, 2.069377661257499), (1.5704225352112675, 0.6165005711539028), (0.6895539906103286, 0.8890873624661702)]}
    # test model
    #summaries={0: [(2.370599806229999, 2.151819333244131), (1.44408561110948, 1.3954750404648795), (0.8069346173042483, 0.9080833907793415), (1.0925985731481753, 1.6095715621343851), (0.649804762044567, 0.7830198704305495), (0.19946566454302575, 0.47740583596198444)], 1: [(4.384615384615385, 2.1218383072699174), (2.243862520458265, 1.445503564473663), (1.0837672965332539, 0.9705361040035383), (3.1715518524029163, 2.0636162460686944), (1.5545305758071715, 0.6263395815559045), (0.6940931409016515, 0.8926831792889001)]}
    #testSet=[3, 3, 0, 1, 1, 2,1,4,9,0]
    """iiio=0
    while iiio < len(testSet):
        if testSet[iiio][4]==0:
            testSet.pop(iiio)
            iiio=iiio-1
        iiio=iiio+1   
