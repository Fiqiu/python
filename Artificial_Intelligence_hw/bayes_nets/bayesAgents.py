...
def constructBayesNet(gameState):
    ...
    obsVars = []
    edges = [(X_POS_VAR,FOOD_HOUSE_VAR),(Y_POS_VAR,FOOD_HOUSE_VAR),(Y_POS_VAR,GHOST_HOUSE_VAR),(X_POS_VAR,GHOST_HOUSE_VAR)]
    variableDomainsDict = {}
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
...
def fillObsCPT(bayesNet, gameState):
    ...
    def house(obsPos):
        for house in gameState.getPossibleHouses():
            #print util.manhattanDistance(obsPos,house)
            if util.manhattanDistance(obsPos,house)<3:
                adjcenthouse=house
        if adjcenthouse==bottomLeftPos:
            return BOTTOM_LEFT_VAL
        if adjcenthouse ==topRightPos:
            return TOP_RIGHT_VAL
        if adjcenthouse==topLeftPos:
            return TOP_LEFT_VAL
        if adjcenthouse == bottomRightPos:
            return BOTTOM_RIGHT_VAL
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsFactor=bn.Factor([obsVar],HOUSE_VARS,bayesNet.variableDomainsDict())
            for branch in obsFactor.getAllPossibleAssignmentDicts():
                if house(obsPos)==branch[FOOD_HOUSE_VAR] and branch[GHOST_HOUSE_VAR]== house(obsPos):
                    probr=PROB_FOOD_RED
                    probb=1-probr
                    probelse=0
                elif house(obsPos)==branch[FOOD_HOUSE_VAR] :
                    probr=PROB_FOOD_RED
                    probb=1-probr
                    probelse=0
                elif branch[GHOST_HOUSE_VAR]== house(obsPos):
                    probr=PROB_GHOST_RED
                    probb=1-probr
                    probelse=0
                else:
                    probr=0
                    probb=0
                    probelse=1
                if branch[obsVar]==BLUE_OBS_VAL:
                    obsFactor.setProbability(branch,probb)
                elif branch[obsVar]==RED_OBS_VAL:
                    obsFactor.setProbability(branch,probr)
                else:
                    obsFactor.setProbability(branch,probelse)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for i in factor.getAllPossibleAssignmentDicts():
        p.append(factor.getProbability(i))
    for i in factor.getAllPossibleAssignmentDicts():
        if max(p)==factor.getProbability(i):
            k=i
    return k
    "*** YOUR CODE HERE ***"
    util.raiseNotDefined()
