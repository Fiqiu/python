...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        distance=[]
        for pos in Ghostpos:
            distance.append(self.distancer.getDistance(pos,pacmanPosition))
        minipos=Ghostpos[distance.index(min(distance))]
        successors=[]
        for action in legal:
            successors.append(Actions.getSuccessor(pacmanPosition,action))
        distance2=[]
        for successor in successors:
            distance2.append(self.distancer.getDistance(successor,minipos))
        return legal[distance2.index(min(distance2))]